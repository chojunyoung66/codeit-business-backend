...
describe("게시글 저장소 테스트 (Mock)", () => {
    describe("findAll", () => {
        ...
        test("같은 시간의 게시글들도 정렬된다", async () => {
            ...
            expect(articles.map((a) => a.id)).toEqual(expect.arrayContaining([1, 2, 3]));
        });
    });
    describe("findById", () => {
    ...
    describe("createArticle", () => {
        ...
        test("생성된 게시글의 createdAt과 updatedAt이 설정된다", async () => {
            ...
            expect(article.createdAt.getTime()).toBeLessThanOrEqual(afterCreate.getTime());
            expect(article.updatedAt.getTime()).toBeGreaterThanOrEqual(beforeCreate.getTime());
            expect(article.updatedAt.getTime()).toBeLessThanOrEqual(afterCreate.getTime());
        });
    ...
    describe("updateArticle", () => {
        ...
        test("존재하지 않는 게시글을 수정하면 에러를 던진다", async () => {
            const repo = createArticleRepoMock([]);
            await expect(repo.updateArticle(999, { title: "수정" })).rejects.toThrow("Article not found");
        });
    ...
    describe("deleteArticle", () => {
        ...
        test("존재하지 않는 게시글을 삭제하면 에러를 던진다", async () => {
            const repo = createArticleRepoMock([]);
            await expect(repo.deleteArticle(999)).rejects.toThrow("Article not found");
        });
        test("여러 게시글 중 하나만 삭제하면 나머지는 유지된다", async () => {
            ...
            expect(remaining.map((a) => a.id)).toEqual(expect.arrayContaining([1, 3]));
        });
