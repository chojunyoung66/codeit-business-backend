...
describe("게시글 서비스 테스트", () => {
  describe("getArticles", () => {
    test("모든 게시글을 조회한다", async () => {
      ...
      const fakeArticleRepo = {
        findAll: jest.fn().mockResolvedValue(fakeArticles),
        findById: jest.fn(),
  ...
  describe("getArticleById", () => {
    test("ID로 게시글을 조회한다", async () => {
      ...
      const fakeArticleRepo = {
        findAll: jest.fn(),
        findById: jest.fn().mockResolvedValue(fakeArticle),
        createArticle: jest.fn(),
      };
  ...
  describe("createArticle", () => {
    test("새로운 게시글을 생성한다", async () => {
      ...
      const fakeArticleRepo = {
        findAll: jest.fn(),
        findById: jest.fn(),
        createArticle: jest.fn().mockResolvedValue(fakeArticle),
      };
    ...
    test("DB 에러 발생 시 에러를 전파한다", async () => {
      ...
      const fakeArticleRepo = {
        findAll: jest.fn(),
        findById: jest.fn(),
        createArticle: jest.fn().mockRejectedValue(dbError),
      };
      const { createArticle } = createArticleService(fakeArticleRepo);
      await expect(
        createArticle({
  ...
  describe("updateArticle", () => {
    ...
    test("게시글 업데이트 - title만 제공하면 기존 article을 반환한다", async () => {
      ...
      const fakeArticleRepo = {
        ...
        findById: jest.fn().mockResolvedValue(fakeArticle),
        createArticle: jest.fn(),
        updateArticle: jest.fn().mockResolvedValue(fakeArticle),
  ...
  describe("deleteArticle", () => {
    ...
    test("게시글 삭제 - 정상 삭제 후 void를 반환한다", async () => {
      ...
      const fakeArticleRepo = {
        ...
        deleteArticle: jest.fn().mockResolvedValue(fakeArticle),
      };
      const { deleteArticle } = createArticleService(fakeArticleRepo);
