...
describe("인증 서비스 테스트", () => {
  describe("signInService", () => {
    test("이메일과 비밀번호가 일치하면 JWT 토큰을 반환한다", async () => {
      ...
      const fakeToken = "asd";
      const fakeFindUserByEmail = jest.fn().mockResolvedValue(fakeUser);
      const fakeJwtUtil = {
    ...
    test("사용자가 존재하지 않으면 에러를 던진다", async () => {
      const fakeFindUserByEmail = jest.fn().mockResolvedValue(null);
      const fakeJwtUtil = {
        sign: jest.fn(),
      };
      const fakeBcryptUtil = {
    ...
    test("bcryptUtil.compare가 실패하면 에러를 전파한다", async () => {
      ...
      const fakeBcryptUtil = {
        compare: jest.fn().mockRejectedValue(new Error("bcrypt 비교 실패")),
        hash: jest.fn(),
      };
      const { signInService } = createAuthService(
        {
  ...
  describe("signUpService", () => {
    test("새 사용자를 생성하고 JWT 토큰을 반환한다", async () => {
      ...
      const fakeCreateUser = jest.fn().mockResolvedValue(newUser);
      const fakeFindUserByEmail = jest.fn().mockResolvedValue(null);
      const fakeJwtUtil = {
        sign: jest.fn().mockReturnValue(fakeToken),
      };
    ...
    test("이미 등록된 이메일이면 에러를 던진다", async () => {
      ...
      const fakeFindUserByEmail = jest.fn().mockResolvedValue(existingUser);
      const fakeJwtUtil = {
        sign: jest.fn(),
      };
      const fakeBcryptUtil = {
    ...
    test("bcryptUtil.hash가 실패하면 에러를 전파한다", async () => {
      ...
      const fakeBcryptUtil = {
        compare: jest.fn(),
        hash: jest.fn().mockRejectedValue(new Error("bcrypt 해시 실패")),
      };
      const { signUpService } = createAuthService(
        {
          findUserByEmail: fakeFindUserByEmail,
